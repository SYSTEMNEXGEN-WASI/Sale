...
namespace Core.CRM.ADO
{
    ...
    //            sql = sql + "where A.CompCode = '" + dealerCode + "'";
    ...
    //    public static List<VoucherVM> LoadCSGrid(string leadId, string dealerCode)
    //    {
    ...
    //        dt = sysfun.GetData("Select [" + code + "] from AccountCodeSetup where DealerCode = '" + DealerCode + "'");
    ...
    //    }        
    //    public static bool Insert_Gvoucher(List<GVouMasterVM> model2,string type)
    //    {
    //        int count = 1;
    ...
    public class VoucherMethods
    {
        ...
        public static bool GetNewVoucherNoReceipt(string sTableName, string sColumn, int NoOfChar, string dealerCode, string VouchNo, ref string newVouch)
        {
            ...
            if (sysfun.CodeExists("ProdRecMaster", "VoucherNo", VouchNo, dealerCode, ref ds))
            {
                newVouch = VouchNo;
                return false;
        ...
        public static List<VoucherVM> LoadBookingOrderGrid(string leadId, string dealerCode, string ChassisNo)
        {
            ...
            dt = DataAccess.getDataTable("SP_Get_BO_VoucherDetail", dsParamInv, CConn.CConnection.GetConnectionString());
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                //  string Customer = dsReceipt.Tables[0].Rows[0]["CusDesc"].ToString().Trim();
                //DataRow[] dr = dsReceipt.Tables[0].Compute("SUM(ReceiptAmount)","");
                double ReceiptAmount = SysFunction.CustomCDBL(dt.Rows[0]["Amount"].ToString().Trim());
                //Convert.ToDouble(dt.Compute("SUM(Amount)", ""));
                //double ReceiptAmount = Convert.ToDouble(dt.Rows[0]["ReceiptAmount"]);
                string Naration = "Type  : " + dt.Rows[0]["Type"].ToString().Trim()
                + " | " + "Chassis No : " + dt.Rows[0]["ChasisNo"].ToString().Trim()
                + " | " + "Product : " + dt.Rows[0]["ProdDesc"].ToString().Trim();
                string Acc = dt.Rows[0]["VehicleStock"].ToString().Trim();
                string AccountCode = dt.Rows[0]["VendorPayable"].ToString();
                string VehExpDesc = GetAccounttitle(AccountCode, dealerCode);
                foreach (DataRow dr in dt.Rows)
                {
                    ReceiptAmount = SysFunction.CustomCDBL(dr["Amount"].ToString().Trim());
                    Naration = "Type  : " + dr["Type"].ToString().Trim()
                + " | " + "Chassis No : " + dr["ChasisNo"].ToString().Trim()
                + " | " + "Product : " + dr["ProdDesc"].ToString().Trim();
                    AddDebitAmount(ReceiptAmount, AccountCode, Naration, dealerCode);
                    AddCreditAmount(ReceiptAmount, Acc, Naration, GetAccounttitle(Acc, dealerCode));
                   
                  
                }
                //  AddDebitAmount(ReceiptAmount, Acc, Naration, dealerCode);

                double Amount = SysFunction.CustomCDBL(dt.Compute("SUM(Amount)", ""));
        ...
        public static List<VoucherVM> LoadVehicleReceiptGrid(string leadId, string dealerCode, string ChassisNo)
        {
            List<VoucherVM> lst = new List<VoucherVM>();
            ...
            dt = DataAccess.getDataTable("SP_Get_VehicleAccountCodeDetail", dsParamInv,CConn.CConnection.GetConnectionString());
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                + " | " + "Product : " + dr["ProdDesc"].ToString().Trim();
                    AddDebitAmount(ReceiptAmount, Acc, Naration, dealerCode);
                    AddCreditAmount(ReceiptAmount, AccountCode, Naration, VehExpDesc);
                }
                //  AddDebitAmount(ReceiptAmount, Acc, Naration, dealerCode);
        ...
        public static List<VoucherVM> LoadCSGrid(string leadId, string dealerCode)
        {
            ...
            if (dsReceipt.Tables[0].Rows.Count > 0)
            {
                ...
                AddDebitAmount(ReceiptAmount, Account, Naration, dealerCode);
        ...
        private static string GetAccountCode(string code)
        {
            ...
            dt = sysfun.GetDatas("Select [" + code + "] from AccountCodeSetup where DealerCode = '" + DealerCode + "'", "BMS0517ConnectionString");
        ...
        public static bool Insert_Gvoucher(List<GVouMasterVM> model2, string type, ref string msg)
        {
            int count = 1;
            ...
            try
            {
                ...
                foreach (var item in model2)
                {
                    ...
                    dt = sysfun.GetDatas(sql, "FAMSConnectionString");
                ...
                else if (type == "CSI")
                {
                    ...
                    IQuery = "Update VehicleSaleMaster set VoucherNo ='" + model2.FirstOrDefault().VouchNo + "' , VoucherFlag = 'Y' " +
                              "Where DealerCode='" + model2.FirstOrDefault().CompCode + "' and TransCode ='" + ReceiptNo + "'";
