...
namespace Core.CRM.ADO
{
    public class MasterMethods
    {
        ...
        public static string Get_VehRecTypeData(string dealerCode)
        {
            ...
            try
            {
                string sql = "Select A.VehRecCode , A.VehRecDesc from VehicleReceiptType A where A.DealerCode = '" + "COMON" + "'";
        ...
        public static string Get_DocumentTypeData(string dealerCode)
        {
            ...
            try
            {
                string sql = "Select A.DocChkListCode , A.DocChkListDesc from DocumentCheckList A Where A.DealerCode = '" + "COMON" + "'";
        ...
        public static string Get_ServiceDetail(string dealerCode)
        {
            ...
            try
            {
                string sql = "Select V.VehExpCode , V.VehExpDesc,V.AccountCode,V.DefaultValue from VehExpHead V Where V.DealerCode = '" + "COMON" + "'";
        ...
        public static string Get_AccountDetail(string dealerCode)
        {
            string json = "";
            var Serializer = new JavaScriptSerializer();
            List<AccountTransactionVM> lst = new List<AccountTransactionVM>();
            try
            {
                string sql = "Select isnull(s.SaleSecurity,'')SaleSecurity,isnull(s.SaleBankGuarantee,'') SaleBankGuarantee, " +
"(Select rtrim(A.DetailDesc) as AccountTitle from fams..GDetail A where A.CompCode = s.DealerCode and A.contacccode + '-' + A.SubCode + '-' + A.subsubcode + '-' + A.loccode + '-' + A.DetailCode = s.SaleSecurity)SaleSecurityDesc,"+
"(Select rtrim(A.DetailDesc) as AccountTitle from fams..GDetail A where A.CompCode = s.DealerCode and A.contacccode + '-' + A.SubCode + '-' + A.subsubcode + '-' + A.loccode + '-' + A.DetailCode = s.SaleSecurity )SaleBankGuaranteeDesc"+
  " From AccountCodeSetup s  where DealerCode = '"+dealerCode+"'  ";
                ...
                if (dt.Rows.Count > 0)
                {
                    lst = EnumerableExtension.ToList<AccountTransactionVM>(dt);
                }
                json = Serializer.Serialize(lst);
            }
            catch (Exception ex)
